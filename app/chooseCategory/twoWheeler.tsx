...
export default function TwoWheelerScreen() {
  ...
  const endIndex = Math.min(startIndex + ITEMS_PER_PAGE, allCurrentQuestions.length);
  const paginatedQuestions = useMemo(() => allCurrentQuestions.slice(startIndex, endIndex), [allCurrentQuestions, startIndex, endIndex]);
      ...
      >
                ...
                <View style={[styles.card, { zIndex: 10 }]}>
                  ...
                  <View style={styles.optionGrid}>
                    <View style={styles.optionCell}>
                      <Text style={[styles.optionText, show && item.correctIndex === 0 && styles.optionCorrect]}>
                        {isNepali ? unicodeToAakriti(`${optionPrefixes[0]} ${stripPrefix(item.opts[0])}`) : `${optionPrefixes[0]} ${stripPrefix(item.opts[0])}`}
                      </Text>
                    </View>
                    <View style={styles.optionCell}>
                      <Text style={[styles.optionText, show && item.correctIndex === 1 && styles.optionCorrect]}>
                        {isNepali ? unicodeToAakriti(`${optionPrefixes[1]} ${stripPrefix(item.opts[1])}`) : `${optionPrefixes[1]} ${stripPrefix(item.opts[1])}`}
                      </Text>
                    </View>
                    <View style={styles.optionCell}>
                      <Text style={[styles.optionText, show && item.correctIndex === 2 && styles.optionCorrect]}>
                        {isNepali ? unicodeToAakriti(`${optionPrefixes[2]} ${stripPrefix(item.opts[2])}`) : `${optionPrefixes[2]} ${stripPrefix(item.opts[2])}`}
                      </Text>
                    </View>
                    <View style={styles.optionCell}>
                      <Text style={[styles.optionText, show && item.correctIndex === 3 && styles.optionCorrect]}>
                        {isNepali ? unicodeToAakriti(`${optionPrefixes[3]} ${stripPrefix(item.opts[3])}`) : `${optionPrefixes[3]} ${stripPrefix(item.opts[3])}`}
                      </Text>
                    </View>
                  </View>
                  ...
                  <View style={{ overflow: 'hidden', zIndex: 1 }}>
                    <View style={styles.answerPill} pointerEvents="none">
                      <Text numberOfLines={2} ellipsizeMode="tail" style={styles.answerPillText}>
                        {isNepali
                          ? unicodeToAakriti(`${letter} ${stripPrefix(item.opts[item.correctIndex])}`)
        ...
        {!isLoading && paginatedQuestions.length === 0 && (
          <View style={{ padding: 16, alignItems: 'center' }}>
            <Text style={{ color: theme.colors.textSecondary }}>{isNepali ? 'प्रश्नहरू चाँडै थपिनेछन्।' : 'Questions will be added soon.'}</Text>
          </View>
        )}
