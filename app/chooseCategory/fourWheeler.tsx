...
export default function FourWheelerScreen() {
  ...
  const sections = useMemo(() => {
    ...
    return [
      { title: 'Section 1', subtitle: 'Knowledge related to vehicular act/regulation.', start: 0, end: allCount, icon: 'document-text-outline' },
      { title: 'Section 2', subtitle: 'Technical or mechanical knowledge of vehicle', start: 0, end: techCount, icon: 'build-outline' },
      { title: 'Section 3', subtitle: 'Conceptual knowledge related to environment pollution', start: 0, end: POLLUTION_QUESTIONS.length, icon: 'leaf-outline' },
      { title: 'Section 4', subtitle: 'Knowledge related to driving', start: 0, end: DRIVE_QUESTIONS.length, icon: 'car-outline' },
      { title: 'Section 5', subtitle: 'Knowledge related to accidental awareness', start: 0, end: accCount, icon: 'medkit-outline' },
      { title: 'Section 6', subtitle: 'Knowledge related to traffic signals', start: 0, end: SIGNAL_QUESTIONS.length, icon: 'alert-circle-outline' },
    ];
  ...
  const questionsForActive = useMemo(() => {
    ...
    let dataA: number[] = [];
    let baseId = 0;
    if (activeSection === 0) { dataQ = QUESTIONS; dataA = ANSWER_KEYS; baseId = 0; }
    else if (activeSection === 1) { dataQ = TECH_QUESTIONS; dataA = TECH_ANSWER_KEYS; baseId = 1000; }
    else if (activeSection === 2) { dataQ = POLLUTION_QUESTIONS; dataA = POLLUTION_ANSWER_KEYS; baseId = 2000; }
    else if (activeSection === 3) { dataQ = DRIVE_QUESTIONS; dataA = DRIVE_ANSWER_KEYS; baseId = 3000; }
    else if (activeSection === 4) { dataQ = ACC_QUESTIONS; dataA = ACC_ANSWER_KEYS; baseId = 4000; }
    else if (activeSection === 5) { dataQ = SIGNAL_QUESTIONS; dataA = SIGNAL_ANSWER_KEYS; baseId = 5000; }
    return dataQ.map((q, idx) => {
      const key = Number.isFinite(dataA[idx]) ? Number(dataA[idx]) : 0;
      return { q: q?.question ?? '', opts: Array.isArray(q?.options) ? q.options : ['', '', '', ''], correctIndex: Math.max(0, Math.min(3, key)), id: baseId + idx };
    });
